...
def cloudsen12_table(dataloader, HImodel, TImodel):
    ...
    for item in range(len(dataloader)):
        ...
        ti_index = sklearn.metrics.fbeta_score(y, yhat, beta=2., average="macro")
...
def KappaSetViz(root, TImodel):
    ...
    for index, item in enumerate(p_errors_files):
        ...
        rgb = np.concatenate([cv2.equalizeHist(band)[...,None] for band in  cv2.split(rgb)], axis=-1)
        ...
        handles = [matplotlib.patches.Patch(color=cmap(i), label=names[i]) for i in range(4)]
        
        # add legend in the bottom
        fig.legend(handles=handles, loc="lower center", ncol=4)
         
        # main title
        fig.suptitle(f"{name} TI: {kappaset_perrors.TI[index]:.3f} - HI: {kappaset_perrors.HI[index]:.3f}")
...
def CloudCatalogueViz(root, TImodel):
    ...
    p_errors_files = [[all_filenames[0][i] for i in p_errors], [all_filenames[1][i] for i in p_errors]]
    ...
    for index, item in enumerate(p_errors_files[0]):
        ...
        handles = [matplotlib.patches.Patch(color=cmap(i), label=names[i]) for i in range(3)]
        
        # add legend in the bottom
        fig.legend(handles=handles, loc="lower center", ncol=4)
         
        # main title
        fig.suptitle(f"{name} TI: {cloudcatalogue_perrors.TI[index]:.3f} - HI: {cloudcatalogue_perrors.HI[index]:.3f}")
...
def CloudSEN12Viz(root, TImodel):
    ...
    training_data = dataloader_cloudsen12.CloudDataset(root=root, type="train", model="seg")
    validation_data = dataloader_cloudsen12.CloudDataset(root=root, type="val", model="seg")
    testing_data = dataloader_cloudsen12.CloudDataset(root=root, type="test", model="seg")
    ...
    for index in range(len(PKcloudSEN12)):
        ...
        elif (len(training_data) + len(validation_data)) <= item < (len(training_data) + len(validation_data) + len(testing_data)):
